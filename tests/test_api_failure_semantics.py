...
def test_action_outcome_and_events_do_not_depend_on_request_emit(monkeypatch):
    with TestClient(appmod.app) as client:
        conv = new_conv(client)
        action = BusinessAction(
            action_id='atomic-action-event',
            kind='merchant.review',
            title='审核',
            description='本地演示',
        )
        appmod.store.save_actions(conv['id'], 's-atomic', [action])

        async def broken_emit(*_args, **_kwargs):
            raise AssertionError('action state transitions must not use a second event transaction')

        monkeypatch.setattr(appmod, 'emit', broken_emit)
        response = client.post(
            '/api/actions/atomic-action-event/decision',
            json={'decision': 'approve', 'note': 'atomic'},
        )
        assert response.status_code == 200
        assert response.json()['status'] == 'simulated'
        updates = [
            event for event in appmod.store.list_events(conv['id'])
            if event['type'] == 'action.updated' and event['payload']['id'] == action.action_id
        ]
        assert [event['payload']['status'] for event in updates] == ['approved', 'simulated']
