...
EcomEvo 的目标不是让客户理解 Agent、Runtime、Planner 或 Verifier，而是让客户知道：**现在处理到哪、还缺什么、为什么这样判断、有没有需要自己确认的事情。**
...
如果信息不足，EcomEvo 会直接说明缺什么，而不是用技术状态码或内部术语描述问题。

### 5. 确认高影响动作

退款、下架、审核结论、风险升级等可能改变真实业务状态的动作，不会因为模型生成了一句话就自动执行。它们会进入明确的确认边界。
...
| 原则 | 在产品里的表现 |
|---|---|
| **资料优先** | 结论尽量回到已提交资料、规则和工具结果，不把模型措辞本身当作事实 |
...
当前 `main` 已切换到客户服务型界面：暖色工作画布、清晰任务入口、客户语言的进度与资料面板，以及更克制的服务状态展示。

设计规范见 [`DESIGN.md`](DESIGN.md)。当前设计主参考 `VoltAgent/awesome-design-md` 中的 Intercom 设计语言，并吸收 Claude 的阅读节奏和少量 Clay 的友好感；内部算法与后端协议不因视觉层改造而改变。

> 仓库中的静态截图资产来自 Browser E2E 历史采集。客户化 UI 已在 `main` 更新，下一轮产品画廊会用新的真实 Chromium 截图整体替换旧的工程控制台截图。
