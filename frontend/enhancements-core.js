(() => {
  ...
  function deriveTurnBusy(events) {
    let accepted = 0;
    let terminal = 0;
    for (const event of Array.isArray(events) ? events : []) {
      const id = Number(event?.id || 0);
      if (event?.type === 'message.accepted') accepted = Math.max(accepted, id);
      if (event?.type === 'answer.ready' || event?.type === 'answer.error') terminal = Math.max(terminal, id);
    }
  ...
  function installUploadGuard() {
    document.addEventListener('change', event => {
      if (event.target?.id === 'fileInput') beginUploadBatch(event.target.files?.length || 0);
    }, true);
  ...
  function restoreConversationTelemetry(payload, expectedConversationId) {
    setTimeout(() => {
      ...
      remoteTurnBusy = deriveTurnBusy(payload?.events);
    ...
    try {
      ...
      if (conversationCreateUrl(input, options) && response.ok) {
        ...
        try {
          ...
          const cid = conversationIdFromRequest(input);
          rememberConversationEvents(payload, cid);
          restoreConversationTelemetry(payload, cid);
        } catch (_) {}
      }
    ...
    function ObservedWebSocket(...args) {
      try {
        ...
        if (match) {
          ...
          const cutoff = Number(eventCutoffs.get(cid) || 0);
  ...
  function renderRuntimePulse() {
    ...
    const modeLabel = mode === 'model_controller' ? '自适应认知' : mode ? '受控本地' : remoteTurnBusy ? '受控运行中' : '等待任务';
    const gap = missing[0] ? String(missing[0]).slice(0, 48) : '';
    const stopText = remoteTurnBusy ? '本轮处理中' : stopLabel(reason);
    const html = `
      <div class="runtime-pulse-head"><div><span class="runtime-live ${sync ? 'ok' : ''}"></span><b>运行质量</b></div><small>证据与权限 · 可审计</small></div>
      <div class="runtime-pulse-grid">
        <div><small>证据状态</small><strong>${evidenceLabel}</strong></div><div><small>工具预算</small><strong>${fmtCost(used)} / ${fmtCost(budget)}</strong></div>
        <div><small>自主步骤</small><strong>${steps || '—'}</strong></div><div><small>停止原因</small><strong>${stopText}</strong></div>
        <div><small>API RTT</small><strong>${fmtLatency(telemetry.apiEwmaMs)}</strong></div><div><small>运行模式</small><strong>${modeLabel}</strong></div>
      </div>
      <div class="runtime-pulse-foot"><span>${gap ? `最先缺口：${gap}` : Number.isFinite(Number(remaining)) ? `剩余预算 ${fmtCost(remaining)}` : `任务用时 ${fmtLatency(runtimeMs)}`}</span><span>${samples > 0 ? `策略样本 ${samples}${Number.isFinite(drift) ? ` · 漂移 ${drift.toFixed(3)}` : ''}` : Number.isFinite(telemetry.counterfactualMs) ? `反事实复核 ${fmtLatency(telemetry.counterfactualMs)}` : '证据优先'}</span></div>`;
  ...
  function installObserver() {
    ...
    new MutationObserver(scheduleUiPass).observe(root, { childList: true, subtree: true });
  }
  ...
  window.addEventListener('unhandledrejection', event => {
    ...
    event.preventDefault();
    resetUploadGuard();
    showInteractionToast('操作未完成，请检查连接后重试');
  });
