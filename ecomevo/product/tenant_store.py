...
class TenantConversationStore(GuardedConversationStore):
    ...
    def finish_job_success(self, job_id: str, *, worker_id: str, session_id: str, actions, answer: str, result: dict):
        completed = super().finish_job_success(
            job_id, worker_id=worker_id, session_id=session_id, actions=actions, answer=answer, result=result
        )
        if completed:
            with self._conn() as c:
                c.execute("UPDATE conversation_jobs SET session_id=? WHERE id=? AND status='succeeded'", (session_id, job_id))
        return completed
