...
if cors_origins:
    app.add_middleware(CORSMiddleware,allow_origins=cors_origins,allow_credentials=False,allow_methods=['GET','POST','PATCH','DELETE','OPTIONS'],allow_headers=['content-type','authorization'])
...
def conversation_get(cid:str):
    ...
    return {**conv,'messages':store.list_messages(cid,limit=200),'message_count':message_count,'history_truncated':message_count>200,'assets':[_public_asset(x) for x in store.list_assets(cid,include_excluded=True)],'events':store.list_events(cid,limit=600),'actions':store.list_actions(cid)}
...
async def asset_upload(file:UploadFile=File(...),conversation_id:str=Form(...)):
    ...
    if len(existing)>=120:raise HTTPException(409,'单个任务最多保留 120 份资料；可永久删除未被引用的资料，或新建任务')
...
async def asset_scope(asset_id:str,req:AssetScopePatch):
    try:current=store.get_asset(asset_id)
    except KeyError:raise HTTPException(404,'资料不存在')
    cid=current.get('conversation_id')
    if cid and store.has_active_turn(cid):raise HTTPException(409,'任务正在处理，结果返回后再调整资料范围')
    try:row=store.set_asset_active(asset_id,req.active,req.reason)
    except KeyError:raise HTTPException(404,'资料不存在')
    if cid:await emit(cid,'asset.scope_updated',{'asset_id':asset_id,'active':bool(row['active']),'reason':row.get('excluded_reason') or ''})
...
async def asset_delete(asset_id:str):
    ...
    cid=current.get('conversation_id')
    if cid and store.has_active_turn(cid):raise HTTPException(409,'任务正在处理，结果返回后再删除资料')
    try:row=store.delete_asset_if_unreferenced(asset_id)
    except KeyError:raise HTTPException(404,'资料不存在')
    if row is None:raise HTTPException(409,'该资料已经进入历史消息、证据或动作记录，不能物理删除；可以排除后续分析以保留审计链')
    if cid:await emit(cid,'asset.deleted',{'asset_id':asset_id,'name':row['name']})
    ...
    async def work():
        ...
        try:
            ...
            for x in result.get('actions',[]):
                ...
                binding=mcp.action_binding(action.kind,{'conversation_id':cid,'session_id':result['session_id'],'domain':result['domain'],'verifier_score':result.get('runtime',{}).get('verifier_score',0),'action_kind':action.kind,'action_id':action.action_id,'risk_level':action.risk_level})
...
async def action_decide(action_id:str,req:ActionDecision):
    ...
    await emit(a['conversation_id'],'action.updated',claimed)
    mcp_server=claimed['payload'].get('mcp_server');mcp_tool=claimed['payload'].get('mcp_tool')
    try:
        if mcp_server and mcp_tool:
            result=await mcp.call_tool(mcp_server,mcp_tool,claimed['payload'].get('arguments',{}));payload_patch.update({'execution_mode':'mcp','execution_result':result,'execution_outcome':'confirmed'});status='executed'
        else:
            payload_patch.update({'execution_mode':'simulation','execution_result':{'simulated':True,'message':'已完成本地演示，不会改变真实业务系统状态'},'execution_outcome':'simulated'});status='simulated'
        row=store.update_action(action_id,status,payload_patch);await emit(a['conversation_id'],'action.updated',row);return row
...
async def conversation_ws(ws:WebSocket,cid:str):
    await ws.accept()
    try:store.get_conversation(cid)
    except KeyError:await ws.close(code=4404);return
    ...
    try:
        ...
        while True:
            try:
                ...
                if recovered:cutoff=max(cutoff,int(recovered.get('id',0)));await ws.send_json(recovered)
                else:await ws.send_json({'type':'heartbeat','conversation_id':cid})
