...
async def _start_durable_worker():
    if getattr(app.state, "durable_worker_task", None):
        return
    stop = asyncio.Event()
    app.state.durable_worker_stop = stop
    app.state.durable_worker_task = asyncio.create_task(job_worker.loop(stop))


@app.on_event("shutdown")
async def _stop_durable_worker():
    stop = getattr(app.state, "durable_worker_stop", None)
    task = getattr(app.state, "durable_worker_task", None)
    if stop:
        stop.set()
    if task:
        task.cancel()
        try:
            ...
            pass
    app.state.durable_worker_task = None
...
def conversation_get(cid: str):
    ...
    return {
        **conversation,
        ...
        "assets": [_public_asset(row) for row in store.list_assets(cid)],
...
async def action_decide(action_id: str, req: ActionDecision):
    ...
    if req.decision == "reject":
        ...
        await emit(action["conversation_id"], "action.updated", row)
        return row
    ...
    if claimed is None:
        raise HTTPException(409, "该操作已经处理过")
    await emit(action["conversation_id"], "action.updated", claimed)
    ...
    try:
        if mcp_server and mcp_tool:
            ...
            payload_patch.update({
                "execution_result": {"queued": True, "message": "已进入业务处理队列（本地演示执行器）"},
                "execution_outcome": "confirmed",
            })
        row = store.update_action(action_id, "executed", payload_patch)
        await emit(action["conversation_id"], "action.updated", row)
        return row
    except httpx.TransportError as exc:
        logger.exception("business action result is uncertain: %s", action_id)
        row = store.update_action(action_id, "uncertain", {
        ...
        row = store.update_action(action_id, "failed", {"execution_error": "下游业务服务明确返回执行失败", "execution_outcome": "failed"})
        await emit(action["conversation_id"], "action.updated", row)
