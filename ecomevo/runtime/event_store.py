...
class EventStore:
    ...
    def _init(self):
        with self._conn() as c:
            ...
            CREATE TABLE IF NOT EXISTS evolution_patches(patch_id TEXT PRIMARY KEY,created_at REAL NOT NULL,payload_json TEXT NOT NULL,fingerprint TEXT);
            ...
            for r in rows:
                try:
                    data=json.loads(r['payload_json']);fp=self._patch_fingerprint(data)
                except Exception:
                    fp=None
                if fp and fp not in seen:
                    c.execute('UPDATE evolution_patches SET fingerprint=? WHERE patch_id=?',(fp,r['patch_id']));seen.add(fp)
                elif fp:
                    c.execute('UPDATE evolution_patches SET fingerprint=NULL WHERE patch_id=?',(r['patch_id'],))
            c.execute('CREATE UNIQUE INDEX IF NOT EXISTS idx_evolution_fingerprint ON evolution_patches(fingerprint) WHERE fingerprint IS NOT NULL')
    @staticmethod
    ...
    def save_patch_if_novel(self, patch:EvolutionPatch)->dict[str,Any]|None:
        ...
        with self._lock,self._conn() as c:
            c.execute('BEGIN IMMEDIATE')
            row=c.execute('SELECT payload_json FROM evolution_patches WHERE fingerprint=? LIMIT 1',(fp,)).fetchone()
            if row:
                try:return json.loads(row['payload_json'])
                except Exception:return {'patch_id':'existing','accepted':patch.accepted,'target':patch.target,'patch':patch.patch}
            c.execute('INSERT INTO evolution_patches(patch_id,created_at,payload_json,fingerprint) VALUES(?,?,?,?)',(patch.patch_id,patch.created_at,patch.model_dump_json(),fp))
        return None
