...
class RuntimeSkill:
    ...
    def posterior_lower_bound(self) -> float:
        # Lightweight conservative lower bound. It is intentionally deterministic and
        # dependency-free; ranking only, never a business-evidence signal.
...
class AdaptiveSkillLibrary:
    ...
    @staticmethod
    def niche_for(domain: str, trigger_terms: Iterable[str], preferred_tools: Iterable[str] = ()) -> str:
    ...
    def get(self, skill_id: str) -> RuntimeSkill | None:
        with self._conn() as c:
            row = c.execute("SELECT * FROM runtime_skills WHERE skill_id=?", (skill_id,)).fetchone()
        return self._decode(row) if row else None

    def list(self, *, domain: str | None = None, status: str | None = None, limit: int = 100) -> list[RuntimeSkill]:
        ...
        q += " ORDER BY updated_at DESC LIMIT ?"; args.append(max(1, int(limit)))
        with self._conn() as c:
            rows = c.execute(q, args).fetchall()
        return [self._decode(r) for r in rows]

    def relevant(self, domain: str, *, query: str = "", missing: Iterable[str] = (), limit: int = 6) -> list[RuntimeSkill]:
        ...
        for skill in candidates:
            ...
            score = 0.46 * skill.posterior_mean + 0.34 * skill.shadow_score + min(0.20, 0.05 * term_hits)
    ...
    def upsert_candidate(
        ...
        *,
        ...
        now = time.time()
        preferred_tools = list(dict.fromkeys(str(x).strip() for x in preferred_tools if str(x).strip()))[:8]
        trigger_terms = list(dict.fromkeys(str(x).strip() for x in trigger_terms if str(x).strip()))[:12]
        niche = self.niche_for(domain, trigger_terms, preferred_tools)
        status = "active" if promote else "shadow"
        # Replay confidence seeds the posterior but does not make it evidence.
        seed_strength = 4.0
        ...
        with self._lock, self._conn() as c:
            c.execute("BEGIN IMMEDIATE")
            current = c.execute(
                "SELECT * FROM runtime_skills WHERE domain=? AND niche=? AND status!='retired' ORDER BY shadow_score DESC,updated_at DESC LIMIT 1",
                (domain, niche),
            ).fetchone()
            if current:
                existing = self._decode(current)
                # Quality-diversity archive: one live representative per pathology niche.
                # A candidate must be measurably better to replace the incumbent.
                ...
                if candidate_rank <= incumbent_rank + 0.015:
                    return existing
                c.execute("UPDATE runtime_skills SET status='retired',updated_at=? WHERE skill_id=?", (now, existing.skill_id))
            skill_id = f"skill-{uuid.uuid4().hex[:12]}"
            c.execute(
                "INSERT INTO runtime_skills VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                (
                    skill_id, domain, niche, str(name)[:120], str(guidance)[:3000],
                    json.dumps(preferred_tools, ensure_ascii=False), json.dumps(trigger_terms, ensure_ascii=False),
                    status, float(shadow_score), alpha, beta, 0, 0, 0, now, now, source_patch_id,
                ),
            )
    ...
    def promote(self, skill_id: str) -> RuntimeSkill | None:
        ...
        if skill.shadow_score < threshold:return skill
        now=time.time()
        with self._lock,self._conn() as c:
            c.execute("BEGIN IMMEDIATE")
            c.execute("UPDATE runtime_skills SET status='active',updated_at=? WHERE skill_id=?",(now,skill_id))
    ...
    def record_outcome(
        ...
        *,
        ...
        now = time.time()
        for skill_id in dict.fromkeys(str(x) for x in skill_ids if str(x)):
            pre=self.get(skill_id)
            if not pre:continue
            policy=self.policy(pre.domain)
            with self._lock, self._conn() as c:
                c.execute("BEGIN IMMEDIATE")
                row = c.execute("SELECT * FROM runtime_skills WHERE skill_id=?", (skill_id,)).fetchone()
                if not row:
                ...
                touched_domains.add(skill.domain)
                ...
                status = skill.status
                if status == "shadow" and uses >= 2 and mean >= 0.68 and skill.shadow_score >= float(policy['promotion_threshold']):
                    status = "active"
                elif status == "active" and uses >= 5 and mean < float(policy['retirement_threshold']):
                    status = "retired"
                c.execute(
                    "UPDATE runtime_skills SET alpha=?,beta=?,uses=?,wins=?,losses=?,status=?,updated_at=? WHERE skill_id=?",
                    (alpha, beta, uses, wins, losses, status, now, skill_id),
                )
                c.execute(
                    "INSERT INTO skill_outcomes(skill_id,session_id,success,score,context_json,created_at) VALUES(?,?,?,?,?,?)",
                    (skill_id, session_id, int(success), float(score), json.dumps(context or {}, ensure_ascii=False, default=str), now),
                )
            updated = self.get(skill_id)
            if updated:
                out.append(updated)
        for domain in touched_domains:
            self._adapt_policy(domain,success=success,skill_used=True)
        return out
