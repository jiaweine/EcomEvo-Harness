...
d=c.get(f"/api/conversations/{conv['id']}").json();assistant=[m for m in d['messages'] if m['role']=='assistant'][-1]
assert assistant['payload']['domain']=='aftersales';assert assistant['payload']['runtime']['event_chain_valid'] is True;assert d['actions']
act=d['actions'][0]
if act['requires_confirmation']:
    done=c.post(f"/api/actions/{act['id']}/decision",json={'decision':'approve','note':'E2E'}).json();assert done['status']=='simulated' and done['payload']['execution_outcome']=='simulated'
